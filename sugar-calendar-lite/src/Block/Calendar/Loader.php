...
class Loader {
	...
	public function enqueue_editor_assets() {
		...
		// The manifest is generated by the block build (`npm run gulp blocks:build`)
		// and is absent on a source-only checkout until assets are built.
	...
	public function ajax_event_popover() { // phpcs:ignore WPForms.PHP.HooksMethod.InvalidPlaceForAddingHooks
		...
		$event_image_html = get_the_post_thumbnail(
			$event_object_id,
			/**
			 * Filter the size of the event image in the popover in Calendar block.
			 *
			 * @since 3.5.0
			 * @since 3.12.0 The size is now used to render a responsive `<img>` tag (with srcset), instead of a CSS background image.
			 *
		...
		// Default popover CTA; add-ons override it via the filter below.
		$event_id = isset( $_POST['event_id'] ) ? absint( $_POST['event_id'] ) : 0;

		$cta = [
			'label'  => __( 'View Event', 'sugar-calendar-lite' ),
			'anchor' => '',
			'type'   => 'view',
		];

		/**
		 * Filter the call-to-action shown in the Calendar block event popover.
		 *
		 * @since 3.12.0
		 *
		 * @param array $cta             CTA data: { label (string), anchor (string), type (string) }.
		 * @param int   $event_object_id The event's WordPress post ID.
		 * @param int   $event_id        The Sugar Calendar event ID.
		 */
		$cta = (array) apply_filters( 'sugar_calendar_block_popover_event_cta', $cta, $event_object_id, $event_id );

		wp_send_json_success(
			[
				'description' => wp_kses_post( $event_description ),
				'image'       => empty( $event_image_html ) ? false : wp_kses( $event_image_html, Helpers::get_event_image_allowed_html() ),
				'cta'         => $cta,
			]
		);
	}
