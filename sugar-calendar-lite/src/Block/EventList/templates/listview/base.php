...
<div class="<?php echo esc_attr( $view_class ); ?>">
	...
	foreach ( $period as $day ) {
		...
		foreach ( $events[ $day->format( 'Y-m-d' ) ] as $event ) {
			...
			// Get the event featured image HTML (responsive, with srcset/sizes).
			$event_image_html = '';

			if ( $event_view->should_display_featured_image() ) {
			...
			<div
				...
				<div class="sugar-calendar-event-list-block__listview__event__body">
					...
					<?php if ( $event_image_html ) : ?>
						<div class="sugar-calendar-event-list-block__listview__event__body__image">
							<div class="sugar-calendar-event-list-block__listview__event__body__image__container">
								<?php echo wp_kses( $event_image_html, Helpers::get_event_image_allowed_html() ); ?>
							</div>
						</div>
					<?php endif; ?>
				</div>
