...
final class Requirements_Check {
	...
	private function load() {
		...
		if ( ! class_exists( $class ) ) {
			require_once dirname( $this->file ) . '/sugar-calendar-plugin.php';
		}
		...
		if ( class_exists( $class ) ) {
			...
			require_once SC_PLUGIN_DIR . 'vendor/woocommerce/action-scheduler/action-scheduler.php';

			// Autoload
			require_once SC_PLUGIN_DIR . 'vendor/autoload.php';
	...
	public function install() {
		...
		add_option( 'sugar_calendar_activated_time', time(), '', false );

		// Store the timestamp of the first plugin activation by license type.
		$license_type = is_readable( SC_PLUGIN_DIR . 'includes/pro/Pro.php' ) ? 'pro' : 'lite';
		$activated    = get_option( 'sugar_calendar_activated', [] );

		if ( empty( $activated[ $license_type ] ) ) {
			$activated[ $license_type ] = time();
			update_option( 'sugar_calendar_activated', $activated );
		}

		// Add option to trigger first-activation logic later on.
		add_option( 'sugar_calendar_is_first_activation', true );
